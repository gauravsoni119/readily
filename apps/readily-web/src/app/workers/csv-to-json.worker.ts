...
addEventListener('message', ({ data }) => {
  ...
  const normalizedData = response.reduce(
    (db: ReadilyDataBase, res) => {
      if (res['exclusiveShelves']) {
        updateExclusiveShelf(res, db);
      }
      if (res['bookshelves']) {
        const shelves = filterNonExclusiveShelf(res['bookshelves'] as string);
        const exclusiveShelfName = getExclusiveShelfName(
          res['bookshelves'] as string
        );
        shelves.forEach((shelf) => {
          const otherShelf = db.shelves.others.find(
            (existingShelf) => existingShelf.name === shelf
          );
          if (otherShelf) {
            otherShelf.books.push(res);
            ++otherShelf.totalCount;
          } else {
            db.shelves.others.push({
              name: shelf,
              totalCount: 1,
              books: [res],
            });
          ...
          if (exclusiveShelfName) {
            const existOtherShelf = db.shelves.exclusiveShelves[
              exclusiveShelfName
            ].otherShelves.find((exclusiveOtherShelf) => {
            ...
            if (existOtherShelf) {
              ...
              db.shelves.exclusiveShelves[exclusiveShelfName].otherShelves.push(
                { name: shelf, totalCount: 1, books: [res] }
              );
            }
          }
        });
...
const updateExclusiveShelf: (res: any, db: ReadilyDataBase) => void = (
  res,
  db
) => {
  const shelfName: keyof typeof EXCLUSIVE_SHELVES = res['exclusiveShelves'];
  const exclusiveShelf =
    db.shelves.exclusiveShelves[EXCLUSIVE_SHELVES[shelfName]];
  exclusiveShelf.books.push(res);
  ++exclusiveShelf.totalCount;
  db.totalPagesRead +=
    shelfName === 'to-read' ? parseInt(res.numberOfPages, 10) || 0 : 0;
};
...
const getExclusiveShelfName = (
  shelves: string
): keyof ExclusiveShelves | undefined => {
